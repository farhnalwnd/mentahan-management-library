        ...
        <div class="max-w-7xl mx-auto sm:px-1 lg:px-5">
            <div class="dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="container">
                        <h2>Edit Buku</h2>
                        <form action="{{ route('admin.buku.update', $bukus->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="nama_buku">Nama Buku:</label>
                                <input type="text" class="form-control mt-2 mb-3" id="nama_buku" name="nama_buku" value="{{ $bukus->nama_buku }}" required>
                            </div>
                            <div class="form-group">
                                <label for="kategori_id">Kategori:</label>
                                <select class="form-control mt-2 mb-3" id="kategori_id" name="kategori_id" required>
                                    @foreach($kategoris as $kategori)
                                        <option value="{{ $kategori->id }}" {{ $kategori->id == $bukus->kategori_id ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control mt-2 mb-3" id="jumlah" name="jumlah" value="{{ $bukus->jumlah }}" required>
                            </div>
                            <div class="form-group">
